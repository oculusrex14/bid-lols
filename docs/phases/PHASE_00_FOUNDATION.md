...
- **FR-3** `allowedHosts` + `normalizeHost` retained; `www.` normalizes to apex; any other unknown host gets 404. The legacy cross-domain 302 brand-host mapping is removed (legacy `/$site` paths 308 → `/` on the same host).
...
- **FR-5** Production without a valid `DATABASE_URL` fails to start with an explicit error (no PGLite fallback); the PGLite self-migrating loop remains for dev/preview only.
...
- **AC-1 Legacy surface gone:** the route files for `bid`, `checkout.$orderId`, `manage.$token`, `listing.$id`, `activity`, `crown`, `spec` do not exist; `grep -rniE 'hype|crown|oracle|swap_count|manage_token' src server` returns no hits in active code; in dev, those URLs return 404 (legacy `/$site/*` paths return 308 → `/`).
...
- **AC-3 Legacy data intact:** post-deploy prod row counts of `listings`, `orders`, `activity`, `crown_*` equal the pre-deploy snapshot (recorded in completion notes); `grep -ciE 'drop |alter ' migrations/0009_foundation.sql` = 0.
...
- **AC-8 Repo hygiene:** `git ls-files .vercel` → 0; tracked `screenshots/` contains only the documented smoke baselines; `.gitignore` has no duplicate env patterns and covers generated outputs; the 9 dead deps absent from `package.json` with `typecheck` green; `attachments/image.png` untracked; `supabase/` removed (verified unused) or justified in completion notes.
- **AC-9 Grok chrome gone:** `public/__grok` absent; no `grok-pwa` middleware/scripts, `PreviewHostBridge`, `MODE_BOOT_SCRIPT`, `p2p.ts`, `VITE_PUBLIC_HOSTNAME`, `VITE_STUN_URLS`, `GROK_AUTH_ISSUER`, `VITE_AUTH_ENABLED` references in `src`/`server`/`scripts`; `grep -c 'dangerouslySetInnerHTML' src -r` = 0.
...
- **AC-11 Pure build:** `npm run build` with `DATABASE_URL` unset completes with zero DB connections and no PGLite file in the output; `package.json` `build` script contains no `db:migrate`/`copy-pglite` step.
...
- **AC-14 Domains live:** all four apex domains return 200 at `/` with their distinct coming-next `<title>`; `www.` variants normalize; an unknown host returns 404; `culturebid.lol` apex connectivity verified working from prod (dev-env DNS anomaly noted and cleared, or documented as environment-only).
- **AC-15 Analytics tests pass:** (a) second page impression in one session increments `views` only; (b) outbound click increments `listings.clicks` and never `visits`; (c) no multiplier in the display import graph (`grep -rn 'hype' src` = 0); (d) scripted session against PGLite yields exact `views`/`visits`/`clicks` deltas.
...
- **AC-17 Design foundation:** one `SiteShell` + per-domain config object drives all four surfaces (code review); `Button` has a loading state; no raw hex color literals in component files outside `styles.css` (grep).
