import { a as quoteSwapFee, i as isSiteId } from "./sites-BLTVPnCd.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/board-fns-eAopH5o4.js
...
async function fetchListing(id) {
	const rows = await (await getSql()).query(`select id, site, url, title, tagline, team, socials, bid_cents, rank, clicks, swap_count,
...
var listingSelect = `id, site, url, title, tagline, team, socials, bid_cents, rank, clicks, swap_count,
  last_bid_at::text as last_bid_at, created_at::text as created_at`;
...
var trackView = createServerFn({ method: "POST" }).validator(siteSchema.parse).handler(trackView_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
