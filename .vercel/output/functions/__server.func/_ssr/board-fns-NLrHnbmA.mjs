import { a as isSiteId, i as SITE_IDS, s as quoteSwapFee } from "./sites-aEGgv7RZ.mjs";
import { n as TSS_SERVER_FUNCTION, t as createServerFn } from "./ssr.mjs";
import { a as object, i as number, n as array, o as string, t as _enum } from "../_libs/zod.mjs";
import { i as hypeMultiplier, n as displayCount, t as HYPE_LOCK_DAILY_VISITS } from "./hype-0lUaUoAU.mjs";
import { a as urlKey, n as normalizeUrl, t as clampSocials } from "./socials-CEYLul7Z.mjs";
import { t as clampValues } from "./values-CJ6qqPr1.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/board-fns-NLrHnbmA.js
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			"/migrations/0002_boards.sql": _0002_boards_default,
			"/migrations/0003_hype.sql": _0003_hype_default,
			"/migrations/0004_socials.sql": _0004_socials_default,
			"/migrations/0005_culture.sql": _0005_culture_default
		});
...
async function fetchListing(id) {
	const rows = await (await getSql()).query(`select id, site, url, title, tagline, team, socials, values, bid_cents, rank, clicks, swap_count,
...
var listingSelect = `id, site, url, title, tagline, team, socials, values, bid_cents, rank, clicks, swap_count,
  last_bid_at::text as last_bid_at, created_at::text as created_at`;
async function ensureSiteStats(site) {
	const sql = await getSql();
	await sql.query(`insert into site_stats (site, views, visits, visits_today, visits_day, launched_at, hype_locked)
     values ($1, 0, 0, 0, current_date, now(), false)
     on conflict (site) do nothing`, [site]);
	await sql.query(`update site_stats
     set visits_today = 0, visits_day = current_date
     where site = $1 and visits_day < current_date`, [site]);
}
...
function publicHype(row) {
	const visitsTodayReal = Number(row.visits_today);
	const viewsReal = Number(row.views);
	const locked = Boolean(row.hype_locked) || visitsTodayReal >= 8e3;
...
async function loadHype(site) {
	const sql = await getSql();
	await ensureSiteStats(site);
	const row = (await sql.query(`select site, views, visits, visits_today, visits_day::text as visits_day,
            launched_at::text as launched_at, hype_locked
     from site_stats where site = $1`, [site]))[0];
	if (!row) return {
		visitsToday: 0,
		totalViews: 0
	};
	const visitsTodayReal = Number(row.visits_today);
	if (!row.hype_locked && visitsTodayReal >= 8e3) {
		await sql.query(`update site_stats set hype_locked = true where site = $1`, [site]);
		row.hype_locked = true;
	}
	return publicHype(row);
}
async function bumpVisits(site) {
	const sql = await getSql();
	await ensureSiteStats(site);
	await sql.query(`update site_stats
     set visits = visits + 1,
     ...
     where site = $1`, [site, HYPE_LOCK_DAILY_VISITS]);
}
...
var getPortal = createServerFn({ method: "GET" }).handler(getPortal_createServerFn_handler, async () => {
	...
	const listingRows = await sql.query(`select * from (
     ...
     where rn <= 3`);
	const statRows = await sql.query(`select site,
            count(*)::int as count,
            coalesce(sum(bid_cents), 0)::bigint as pool,
            coalesce(sum(clicks), 0)::bigint as clicks
     from listings
     where bid_cents > 0
     group by site`);
	const hypeRows = await sql.query(`select site, views, visits, visits_today, visits_day::text as visits_day,
            launched_at::text as launched_at, hype_locked
     from site_stats`);
	...
	function pack(site) {
		...
		const hype = hypeRow ? publicHype(hypeRow) : {
			visitsToday: 0,
			totalViews: 0
		};
		return {
...
var createBidOrder = createServerFn({ method: "POST" }).validator(object({
	...
	if (data.site === "founders" && data.team.trim().length < 2) throw new Error("Founding team names are required.");
	if (data.site === "culture" && data.tagline.trim().length < 2) throw new Error("A short culture statement is required.");
	...
	const { createCashfreeSession } = await import("./cashfree-DVbsko_t.mjs").then((n) => n.t);
...
var getOrder = createServerFn({ method: "GET" }).validator(object({ orderId: string().min(1) }).parse).handler(getOrder_createServerFn_handler, async ({ data }) => {
	...
	const paymentSessionId = String(payload.paymentSessionId ?? `session_${row.id}`);
	const gatewayLive = payload.gatewayLive === true;
	const gatewayMode = payload.gatewayMode === "production" ? "production" : "sandbox";
	return {
		id: row.id,
		site: row.site,
		kind: row.kind,
...
var trackClick = createServerFn({ method: "POST" }).validator(object({ id: string().min(1) }).parse).handler(trackClick_createServerFn_handler, async ({ data }) => {
	const row = (await (await getSql()).query(`update listings set clicks = clicks + 1 where id = $1 returning url, clicks, site`, [data.id]))[0];
...
var trackView_createServerFn_handler = createServerRpc({
	id: "69cfd1c9eb676871b2ed4db048de66e189206315587bc346d376d6cfe83a2db1",
	name: "trackView",
	filename: "src/lib/board-fns.ts"
}, (opts) => trackView.__executeServer(opts));
var trackView = createServerFn({ method: "POST" }).validator(object({ sites: array(siteEnum).min(1).max(3) }).parse).handler(trackView_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
